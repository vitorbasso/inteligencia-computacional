package mnist;

import fullyconnectednetwork.Network;
import fullyconnectednetwork.NetworkTools;
import sun.nio.ch.Net;
import trainset.TrainSet;

import java.io.File;

/**
 * Created by Luecx on 10.08.2017.
 */
public class Mnist {

    public static void main(String[] args) {
        //TRAIN NETWORK
        ///*
        Network network = new Network(784, 70, 35, 10);
        TrainSet set = createTrainSet(0,24999);
        trainData(network, set, 100, 50, 500);
        set = createTrainSet(25000, 49999);
        trainData(network, set, 100, 50, 500);


        TrainSet testSet = createTrainSet(50000,59999);
        testTrainSet(network, testSet, 10);

        /*try{
            network.saveNetwork("net4.txt");
        }catch (Exception e){
            e.printStackTrace();
        }
        //*/
        //TEST NETWORK
        /*
        TrainSet testSet = createTestSet(0, 4999);
        TrainSet testSet2 = createTestSet(5000, 9999);

        try{
           Network network = Network.loadNetwork("net1.txt");
           testTestSet(network, testSet, 10);
           testTestSet(network, testSet2, 10);


        }catch (Exception e){
            e.printStackTrace();
        }
        */


    }

    public static TrainSet createTrainSet(int start, int end) {

        TrainSet set = new TrainSet(28 * 28, 10);

        try {

            String path = new File("").getAbsolutePath();

            MnistImageFile m = new MnistImageFile(path + "/res/trainImage.idx3-ubyte", "rw");
            MnistLabelFile l = new MnistLabelFile(path + "/res/trainLabel.idx1-ubyte", "rw");

            for(int i = start; i <= end; i++) {
                if(i % 100 ==  0){
                    System.out.println("prepared: " + i);
                }

                double[] input = new double[28 * 28];
                double[] output = new double[10];

                output[l.readLabel()] = 1d;
                for(int j = 0; j < 28*28; j++){
                    input[j] = (double)m.read() / (double)256;
                }

                set.addData(input, output);
                m.next();
                l.next();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }

         return set;
    }

    public static TrainSet createTestSet(int start, int end) {

        TrainSet set = new TrainSet(28 * 28, 10);

        try {

            String path = new File("").getAbsolutePath();

            MnistImageFile m = new MnistImageFile(path + "/res/t10k-images.idx3-ubyte", "rw");
            MnistLabelFile l = new MnistLabelFile(path + "/res/t10k-labels.idx1-ubyte", "rw");

            for(int i = start; i <= end; i++) {
                if(i % 100 ==  0){
                    System.out.println("prepared: " + i);
                }

                double[] input = new double[28 * 28];
                double[] output = new double[10];

                output[l.readLabel()] = 1d;
                for(int j = 0; j < 28*28; j++){
                    input[j] = (double)m.read() / (double)256;
                }

                set.addData(input, output);
                m.next();
                l.next();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }

        return set;
    }

    public static void trainData(Network net,TrainSet set, int epochs, int loops, int batch_size) {
        for(int e = 0; e < epochs;e++) {
            net.train(set, loops, batch_size);
            System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>   "+ e+ "   <<<<<<<<<<<<<<<<<<<<<<<<<<");
        }
    }

    public static void trainData(Network net, int set_size, int epochs, int loops, int batch_size) {
        int batches = set_size / batch_size;
        for(int i = 0 ; i < batches ; i ++){
            TrainSet set = createTrainSet(i * batch_size, i * batch_size + batch_size - 1);
            for(int e = 0; e < epochs;e++) {
                net.train(set, loops);
                System.out.println(">>>>>>>>>>>>>>>>>>>>>>>>>   "+ e+ "   <<<<<<<<<<<<<<<<<<<<<<<<<<");
            }
        }
    }

    public static void testTrainSet(Network net, TrainSet set, int printSteps) {
        int correct = 0;
        for(int i = 0; i < set.size(); i++) {

            double highest = NetworkTools.indexOfHighestValue(net.calculate(set.getInput(i)));
            double actualHighest = NetworkTools.indexOfHighestValue(set.getOutput(i));
            if(highest == actualHighest) {

                correct ++ ;
            }
            if(i % printSteps == 0) {
                System.out.println(i + ": " + (double)correct / (double) (i + 1));
            }
        }
        System.out.println("Testing finished, RESULT: " + correct + " / " + set.size()+ "  -> " + (double)correct / (double)set.size() +" %");
    }

    public static void testTestSet(Network net, TrainSet set, int printSteps) {
        int correct = 0;
        for(int i = 0; i < set.size(); i++) {

            double highest = NetworkTools.indexOfHighestValue(net.calculate(set.getInput(i)));
            double actualHighest = NetworkTools.indexOfHighestValue(set.getOutput(i));
            if(highest == actualHighest) {

                correct ++ ;
            }
            if(i % printSteps == 0) {
                System.out.println(i + ": " + (double)correct / (double) (i + 1));
            }
        }
        System.out.println("Testing finished, RESULT: " + correct + " / " + set.size()+ "  -> " + (double)correct / (double)set.size() +" %");
    }
}
